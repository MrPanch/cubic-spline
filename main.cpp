...
int main() {
    ...
    std::vector<double> a(system.GetHeight(), 0);
    get_a(system, a);
    ...
    alpha[0] = - system.GetMatrix()[0][1] / y[0];
    beta[0]  =   system.GetFreeVariables()[0] / y[0];
    ...
    size_t tmp = system.GetWidth(); // =3 !
    x[system.GetWidth() - 1] = beta[system.GetWidth() - 1]; // ТУТ ОШИБКА!
    for(size_t i = system.GetWidth() - 2; i > 0; --i) {
